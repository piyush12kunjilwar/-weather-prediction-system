...
import pandas as pd
import glob
from model_architectures import create_simple_model
...
def train_and_evaluate_model(model, X_train, y_train, X_test, y_test, model_name, sequence_length):
    ...
    val_dataset = create_dataset(X_test, y_test, batch_size=256)
    
    # Train model with larger batch size and fewer epochs
    history = model.fit(
        train_dataset,
        epochs=20,
...
def main():
    """Main function to train and evaluate Model 1."""
    # Create directories
    ...
    sequence_lengths = [8, 16]  # Start with smaller sequence lengths
    results = {}
